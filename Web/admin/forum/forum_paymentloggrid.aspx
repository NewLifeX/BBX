<%@ Page language="c#" Inherits="BBX.Web.Admin.paymentloggrid" Codebehind="forum_paymentloggrid.aspx.cs" %>
...
<table width="100%">
	...
	<tr>
		<td class="vtop rowform">
			  <cc2:TextBox id="Username" runat="server" RequiredFieldType="暂无校验" width="150"></cc2:TextBox>
...
<cc1:datagrid id="DataGrid1" runat="server" OnPageIndexChanged="DataGrid_PageIndexChanged" PageSize="15" >
	<Columns>						
		...
		<asp:TemplateColumn HeaderText="作者">
			<ItemTemplate>
				<a href="../../userinfo.aspx?userid=<%# Eval("authorid").ToString() %>" target="_blank">
					<%# Eval("authorname").ToString()%>
				</a>
			</ItemTemplate>
		</asp:TemplateColumn>					
		<asp:BoundColumn DataField="authorid" HeaderText="authorid"  Visible="false"></asp:BoundColumn>
		<asp:BoundColumn DataField="buydate" HeaderText="时间" ></asp:BoundColumn>				
		<asp:TemplateColumn HeaderText="论坛">
			<ItemTemplate>
				<a href="../../showforum.aspx?forumid=<%# Eval("fid").ToString()%>" target="_blank">
					<%# Eval("forumname").ToString()%>
				</a>
			</ItemTemplate>
		</asp:TemplateColumn>					
		<asp:BoundColumn DataField="fid" HeaderText="fid" Visible="false"></asp:BoundColumn>
		<asp:BoundColumn DataField="amount" HeaderText="售价" ></asp:BoundColumn>
		<asp:BoundColumn DataField="netamount"  HeaderText="作者所得" ></asp:BoundColumn>
	</Columns>
</cc1:datagrid>
<div class="Navbutton">
	...
	&nbsp;&nbsp;<cc1:Button id="DelRec" runat="server" Text=" 删 除 "   ButtonImgUrl="../images/del.gif" Enabled="false"
	 OnClientClick="if(!confirm('你确认要删除所选的积分交易日志记录吗？')) return false;"></cc1:Button>
</div>
<script type="text/javascript">
