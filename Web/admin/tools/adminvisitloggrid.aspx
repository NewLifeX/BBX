<%@ Page language="c#" Inherits="BBX.Web.Admin.adminvisitloggrid" Codebehind="adminvisitloggrid.aspx.cs" %>
...
<script type="text/javascript">
	...
	{
		CheckAll(form);
		checkedEnabledButton(form,'visitid','DelRec');
	}
...
<Columns>						
	 <asp:TemplateColumn HeaderText="<input title='选中/取消' onclick='Check(this.form)' type='checkbox' name='chkall' id='chkall' />">
		...
		<ItemTemplate>
			<input id="visitid" type="checkbox" name="visitid" onclick="checkedEnabledButton(this.form,'visitid','DelRec')" value="<%# Eval("ID").ToString() %>" />
		</ItemTemplate>
	</asp:TemplateColumn>					
	<asp:BoundColumn DataField="ID" HeaderText="visitid [递增]" Visible="false" ></asp:BoundColumn>
	...
	<asp:BoundColumn DataField="others"  HeaderText="其他" ></asp:BoundColumn>
</Columns>
</cc1:datagrid>
<div class="Navbutton">
<input type="radio" name="deleteMode" checked="checked" onclick="changeDeleteModeState(1,this.form)" value="chkall" id="radio1"/>
<input title="选中/取消选中 本页所有Case" onclick="document.getElementById('chkall').click();" type="checkbox" name="chkall1" />全选/取消全选
...
&nbsp;&nbsp;<cc1:Button id="DelRec" runat="server" Text=" 删 除 "   ButtonImgUrl="../images/del.gif" Enabled="false" 
 OnClientClick="if(!confirm('你确认要删除所选的后台访问记录吗？')) return false;"></cc1:Button>
</div>
<script type="text/javascript">
