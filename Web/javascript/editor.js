		...
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				...
				if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'url') {
					buttons[i].onclick = function(e) {bbxcode('unlink');bbxcode('url');};
				} else {
					buttons[i].onclick = function(e) {bbxcode(this.id.substr(this.id.indexOf('_') + 1));};
				}
			}
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				...
				_attachEvent(buttons[i], 'mouseover', function(e) {setEditorTip(BROWSER.ie ? window.event.srcElement.title : e.target.title);});
				if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'url') {
					buttons[i].onclick = function(e) {bbxcode('unlink');bbxcode('url');doane();};
				} else {
					buttons[i].onclick = function(e) {bbxcode(this.id.substr(this.id.indexOf('_') + 1));doane();};
				}
			}
			buttons[i].onmouseout = function(e) {setEditorTip('');};
		}
	...
	if(menu) {
		...
		switch(tag) {
			...
			case 'tbl':
				...
				str += '<div id="tbltips_msg" style="display: none">“[tr=颜色]” 定义行背景<br />“[td=宽度]” 定义列宽<br />“[td=列跨度,行跨度,宽度]” 定义行列跨度<br /><br />快速书写表格范例：<div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|BBX<br />Version:|V1.0<br />[/table]</div>用“|”分隔每一列，表格中如有“|”用“\\|”代替，换行用“\\n”代替。</div>';
				break;
			case 'aud':
...
function getunusedattachlist_dialog() {
	_sendRequest('tools/ajax.ashx?t=getattachlist', getunusedattachlist_callback, false);
}
