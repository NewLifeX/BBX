...
namespace BBX.Web
{
	public class posttopic : PageBase
	{
		...
		public Topic CreateTopic(AdminGroup admininfo, string postmessage, bool isbonus, int topicprice)
		{
			...
			if (!Utils.StrIsNullOrEmpty(text) && Utils.HtmlDecode(text).Trim() != tp.Title)
			{
				tp.Magic = 11000;
			}
			...
			tp.Create(postInfo);
			if (this.canhtmltitle && !Utils.StrIsNullOrEmpty(text) && text != tp.Title)
			{
		...
		private void NormalValidate(AdminGroup admininfo, string postmessage, IUser user)
		{
			...
			if (this.forum.ApplytopicType == 1 && this.forum.PostbytopicType == 1 && !Utils.StrIsNullOrEmpty(this.topictypeselectoptions))
			{
				if (DNTRequest.GetString("typeid").IsNullOrWhiteSpace())
				{
